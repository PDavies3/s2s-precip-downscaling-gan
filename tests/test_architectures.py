...
def test_discriminator_forward_pass(variant_id, sample_batch):
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    netG, netD = get_models(variant_id, sample_batch['dynamic_input'].shape[1],
                             sample_batch['static_input'].shape[1], device)
    ...
    if variant_id in [1, 3]:
        ...
        coarse_interp = F.interpolate(dynamics, size=statics.shape[-2:], mode='bilinear', align_corners=False)
